...
describe('ContactForm', () => {
  ...
  it('shows errors when submitting empty fields', async () => {
    ...
    await expect.element(page.getByText(/message is required/i)).toBeVisible();
  });
  ...
  it('shows error for short message', async () => {
    ...
    await expect.element(page.getByText(/Your message must be a minimum of 10 characters/i)).toBeVisible();
  });
  ...
  it('submits successfully when all validations pass', async () => {
    ...
    await expect.element(page.getByText(/required/i)).toBeNull();
  });
});
